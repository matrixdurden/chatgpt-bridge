...
Authentication:
  A 64-character random token is generated automatically. To provide your own,
...
Do not run the bridge as root. The installer uses sudo only for system files and
systemd; the service itself runs as --service-user.
EOF
}
...
[[ "$SERVICE_USER" != "root" ]] || fail "refusing to run the bridge as root"
...
case "$TOKEN" in
    *$'\n'*|*$'\r'*) fail "CHATGPT_BRIDGE_TOKEN cannot contain newlines" ;;
esac
...
# Conservative systemd hardening that still permits normal development tools.
